	...
	public function compileExpressionFromIndex(enumType:EnumType, enumField:EnumField, exprArgsPassed:Null<Array<TypedExpr>>) {
		...
		return switch (kind) {
			...
			case AsDictionary if (exprArgsPassed != null && providedArgs): { // Redundant null-check for null-safety
					...
					// _index, _hx_name, index — required by Type.enumEq / enumConstructor / enumIndex
					result.addMulti("{ \"_index\": ", Std.string(enumField.index));
					result.addMulti(", \"_hx_name\": \"", enumField.name, "\"");
					result.addMulti(", \"index\": ", Std.string(enumField.index));

					// _params array — required by Type.enumParameters
					// Named "_params" to avoid collision with enum fields named "params".
					result.add(", \"_params\": [");
					for (i in 0...exprArgsPassed.length) {
						if (i > 0)
							result.add(", ");
						result.add(main.compileExpressionOrError(exprArgsPassed[i]));
					}
					result.add("]");

					// named fields — for direct field access by user code
					for (i in 0...exprArgsPassed.length) {
